...
class YindlClient extends events_1.default {
    ...
    start() {
        const socket = net_1.default.connect({ 'host': this.host, 'port': this.port }, this.onConnected.bind(this));
    ...
    onDataReceived(data) {
        ...
        if (type === YINDL_TYPE.Heartbeat_Ack) {
            ...
            const knx_telegram_list = [];
            for (let i = 8; i <= payload.length - 11; i += 11) {
                knx_telegram_list.push(payload.slice(i, i + 11));
            }
            this.onKNXUpdate(knx_telegram_list);
    ...
    login(usr, psw) {
        const buf = Buffer.alloc(usr.length + psw.length + 4);
    ...
    telegram_publish(id, value) {
        ...
        const telegrams = [telegram]; // TODO multiple publish
        const buf = Buffer.alloc(8 + telegrams.length * 11);
    ...
    send(type, payload) {
        ...
        const bcc = bcc_checksum(buf.toString('binary', 4, 15 + payload.length));
    ...
    onKNXUpdate(knx_telegram_list) {
        let output = `新的数据数量: ${knx_telegram_list.length}`;
        for (let i = 0; i < knx_telegram_list.length; i++) {
            const knx_telegram = knx_telegram_list[i];
            const id = parseInt(knx_telegram.readUInt8(3));
            const value = parseInt(knx_telegram.readFloatBE(7));
            this.knx_state[id] = value;
            this.emit('event', {
                'id': id,
                'value': value,
